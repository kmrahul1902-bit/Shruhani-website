...
const escalation = getContent("escalation");

test.describe("escalation page", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/products/escalation");
  });

  test("renders one h1, and it is the page's headline", async ({ page }) => {
    await expect(page.getByRole("heading", { level: 1 })).toHaveCount(1);
    await expect(
      page.getByRole("heading", { level: 1, name: escalation.hero.heading })
    ).toBeVisible();
  });
  ...
  test("renders every section's heading in the mockup's order", async ({
    ...
    const headings = [
      escalation.gap.heading,
      escalation.howItWorks.heading,
      escalation.personaLayers.heading,
      escalation.toolkit.heading,
      escalation.threats.heading,
      escalation.metricsBand.headingPlain,
    ];
  ...
  test("frames the escalation hero animation, decoratively", async () => {
    test.skip(
      true,
