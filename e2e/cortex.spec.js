...
const cortex = getContent("cortex");

test.describe("cortex page", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/products/cortex");
  });

  test("renders one h1, and it is the page's headline", async ({ page }) => {
    await expect(page.getByRole("heading", { level: 1 })).toHaveCount(1);
    await expect(
      page.getByRole("heading", { level: 1, name: cortex.hero.heading })
    ).toBeVisible();
  });
  ...
  test("renders every section's heading in the mockup's order", async ({
    ...
    const headings = [
      cortex.gap.heading,
      cortex.howItWorks.heading,
      cortex.personaLayers.heading,
      cortex.toolkit.heading,
      cortex.threats.heading,
      cortex.metricsBand.headingPlain,
    ];
  ...
  test("frames the cortex hero animation, decoratively", async () => {
    test.skip(
      true,
